...
class LicenseManager:
    ...
    async def async_load(self) -> None:
        ...
        _LOGGER.debug(
            "[Fleet:License] Fingerprint generat: %s...", self._fingerprint[:16]
        )
    ...
    async def async_check_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            _LOGGER.error(
                "[Fleet:License] eroare de rețea la verificare status — %s", err
            )
            return self._status_token
        except Exception as err:  # noqa: BLE001
            _LOGGER.error(
                "[Fleet:License] eroare neașteptată la verificare status — %s", err
            )
            return self._status_token
    ...
    def is_licensed(self) -> bool:
        ...
        if self._status_token and not self._is_status_cache_valid():
            _LOGGER.warning(
                "[Fleet:License] cache-ul de status a expirat — necesită "
    ...
    def _verify_token_signature(self, token: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not isinstance(public_key, Ed25519PublicKey):
                _LOGGER.error("[Fleet:License] cheia publică nu e Ed25519")
                return False
