"""
Funcții utilitare comune pentru integrarea Fleet.
...
def decimal(valoare: Any, zecimale: int = 2) -> float | None:
    """Convertește o valoare la float cu precizie specificată.

    Util pentru preț/litru, consum mediu, etc.
    """
    if valoare is None or valoare == "":
        return None
    try:
        return round(float(valoare), zecimale)
    except (ValueError, TypeError):
        return None
...
def valideaza_campuri_an(
    ...
    limite = {
        ...
        CONF_REMORCA_AN_FABRICATIE: an_max_fabricatie,
    }
...
def sarcina_utila(mma: Any, masa_proprie: Any) -> int | None:
    """Calculează sarcina utilă: MMA - Masa proprie (kg)."""
    m = intreg(mma)
    mp = intreg(masa_proprie)
